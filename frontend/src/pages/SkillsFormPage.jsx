...
const SecondarySkillsForm = () => {
  ...
  useEffect(() => {
    if (form.practice) {
      axios.get(`${BASE_URL}/api/practice-areas/practice/${form.practice}`)
  ...
  useEffect(() => {
    if (form.practiceArea) {
      axios.get(`${BASE_URL}/api/practice-product-technology/practice-area/${form.practiceArea}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`${BASE_URL}/api/profile/${userEmail}`, {
      ...
      await axios.put(
        `${BASE_URL}/api/profile/${userEmail}`,
        updatedProfile,
        {
          withCredentials: true,
          headers: {
