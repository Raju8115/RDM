...
const ProfilePage = ({ userEmail, onLogout }) => {
    ...
    const fetchUserAndProfile = async () => {
      try {
        ...
        if (email) {
          ...
          const profileResponse = await axios.get(`${BASE_URL}/api/profile/${email}`, { withCredentials: true });
  ...
  const fetchUnreadMessages = async () => {
    try {
      ...
      const url = `${BASE_URL}/api/user-messages/${email}/unread`;
  ...
  const handleInboxClick = async () => {
    ...
    try {
      const email = profileData.email ? profileData.email.toLowerCase() : '';
      const url = `${BASE_URL}/api/user-messages/${email}`;
      const response = await axios.get(url, { withCredentials: true });
      const messages = (response.data || []).map(msg => ({ ...msg, read: !!msg.read }));
    ...
    if (unreadMessages.length > 0) {
      await Promise.all(unreadMessages.map(async (msg) => {
        try {
          await axios.put(`${BASE_URL}/api/user-messages/${msg.id}/read`, {}, { withCredentials: true });
        } catch (error) {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL}/api/practices`)
      .then((res) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`${BASE_URL}/api/profile/${email}`, updateData);
      setIsEditing(false);
